...
3. Shift duration is calculated from `Clock In` and `Clock Out`; `Clock Out Date` is used when present, otherwise overnight shifts are inferred when `Clock Out` is earlier than `Clock In`.
4. Break minutes are derived from `Total Duration - Total Excluding Breaks` when explicit break fields are not present.
